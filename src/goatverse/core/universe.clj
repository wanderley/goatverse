...
(defn make-universe
  ...
  - `on-message` is a function called for each event that happens on the
  universe.
  ...
  parameters.  The creator is responsible to handle these events on `on-message`
  callback.
  ...
  - `ws-handler` is a function that handles all connection/message logic (using
    sockets)."
  [{:keys [initial-state tick-rate on-message]}]
  (let [state (atom initial-state)
        main-chan (async/chan (async/sliding-buffer 10))
        ...
        dispatch (fn [& params]
                   (swap! state
                          #(let [next
                                 (apply on-message (into [(first params) %]
                                                         (rest params)))]
